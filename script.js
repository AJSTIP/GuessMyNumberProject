...
function submitFuntion() {
  var x, text;
  x = document.getElementById("sheesh").value;

  if (x < 1 || x > 20) {
    text =
      "That Seems A Little To High, only input numbers between 1 & 20.";
    ...
    text = "No Letters, Letters arent welcome here";
    document.getElementById("response").innerHTML = text;
    document.getElementById("numberGuessed").innerHTML = "Number Guessed: " + x;
    score = score + miss;
    document.getElementById("score").innerHTML = "Score: " + score;
...
function lives() {
  if (x != randomNumber) {
    totalGuesses--;
    console.log("Guesses you have Left " + totalGuesses);
  }
...
function highScore(){
  if (score > bestScore) {
    ...
    alert("You Got The New High Score! Good Job " + bestScore);
  } else {
    alert("You did not get a new High Score, you Should Try Again Though " + score);
  }
}
